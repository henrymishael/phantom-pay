...
import { ProofBadge } from '../../../src/components/ui/ProofBadge'
import { PrivacyModeBadge } from '../../../src/components/ui/PrivacyModeBadge'
...
export default function PaymentPage() {
  ...
  const paymentMutation = useMutation({
    mutationFn: (body: { senderProof?: string }) => apiClient.submitPayment(linkId, body),
  })
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-zinc-950 p-4">
        <div className="w-full max-w-md bg-zinc-900 border border-zinc-800 rounded-xl p-8 text-center space-y-4">
          <h1 className="text-2xl font-bold text-white">Payment link not found</h1>
          <p className="text-zinc-400">This payment link does not exist or has been removed.</p>
        </div>
      </div>
    )
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-zinc-950 p-4">
        <div className="w-full max-w-md bg-zinc-900 border border-zinc-800 rounded-xl p-8 text-center space-y-4">
          ...
          {link.expiresAt && (
            <p className="text-zinc-500 text-sm">Expired on {new Date(link.expiresAt).toLocaleString()}</p>
          )}
        </div>
      </div>
    )
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-zinc-950 p-4">
        <div className="w-full max-w-md bg-zinc-900 border border-zinc-800 rounded-xl p-8 text-center space-y-4">
          ...
          <p className="text-zinc-400">This payment has already been completed</p>
        </div>
      </div>
    )
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-zinc-950 p-4">
        <div className="w-full max-w-md bg-zinc-900 border border-zinc-800 rounded-xl p-8 text-center space-y-6">
          ...
          {paymentMutation.data.proofValid && (
      ...
      <div className="w-full max-w-md bg-zinc-900 border border-zinc-800 rounded-xl p-8 space-y-6">
        ...
        <div className="bg-zinc-800 border border-zinc-700 rounded-lg p-6 space-y-4">
          <div className="flex items-center justify-between">
            ...
            <span className="text-white text-2xl font-bold">{link.amount} {link.token}</span>
          </div>

          {link.description && (
            <div>
              <span className="text-zinc-400 text-sm block mb-1">Description</span>
          ...
          {link.expiresAt && (
            <div>
              ...
              <p className="text-white text-sm">{new Date(link.expiresAt).toLocaleString()}</p>
            </div>
          )}
        ...
        {link.privacyMode === 'verifiable' && (
          <div className="bg-zinc-800/50 border border-zinc-700 rounded-lg p-4 space-y-3">
            <p className="text-zinc-300 text-sm font-medium">Verify your identity (optional)</p>
            <p className="text-zinc-500 text-xs">
              Connect your wallet to prove legitimacy without revealing your identity.
            </p>
            {!connected ? (
              <button
        ...
        {paymentMutation.isError && (
          <div role="alert" className="bg-red-950/40 border border-red-800 rounded-lg px-4 py-3 text-red-400 text-sm">
            {paymentMutation.error instanceof Error ? paymentMutation.error.message : 'Payment failed. Please try again.'}
          </div>
        )}
        ...
        <p className="text-zinc-500 text-xs text-center">
          Your payment will be processed privately. No identity information is exposed.
        </p>
      </div>
    </div>
  )
