    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-zinc-950 px-4">
      <div className="w-full max-w-md text-center space-y-8">
        <div className="space-y-3">
          <h1 className="text-4xl font-bold text-white tracking-tight">PhantomPay</h1>
        ...
        <div className="space-y-4">
          ...
          >
            {loading ? 'Authenticating…' : connected ? 'Sign in with Wallet' : 'Connect Wallet'}
          </button>
          ...
          {error && (
            <p role="alert" className="text-red-400 text-sm bg-red-950/40 border border-red-800 rounded-lg px-4 py-2">
              {error}
            </p>
          )}
        </div>
        ...
        <div className="grid grid-cols-3 gap-4 pt-4 border-t border-zinc-800">
          {['Anonymous payments', 'Session keys', 'No identity exposed'].map(f => (
