...
/** Raw response from the API where id might be named linkId */
interface PaymentLinkResponse extends Omit<PaymentLink, "id" | "status"> {
  id?: string;
  linkId?: string;
  status: "active" | "expired" | "fulfilled" | "deactivated";
}
...
class ApiClient {
  ...
  async getPublicLink(linkId: string): Promise<PaymentLink> {
    const res = await this.request<PaymentLinkResponse>(
      "GET",
      `/payments/links/${linkId}`,
    );
    return {
      ...res,
      id: res.id || res.linkId || "",
  ...
  async getEarnings(): Promise<EarningsResponse> {
    return this.request<EarningsResponse>("GET", "/earnings");
  }

  async getPortfolio(): Promise<PortfolioItem[]> {
    return this.request<PortfolioItem[]>("GET", "/portfolio");
  }
